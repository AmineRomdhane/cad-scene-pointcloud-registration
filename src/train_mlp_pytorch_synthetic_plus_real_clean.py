...
Outputs:
- results/tables/mlp_synthetic_plus_real_clean_results_by_shape.csv
- results/tables/mlp_synthetic_plus_real_clean_results_average.csv
- results/tables/mlp_synthetic_plus_real_clean_training_history_<feature_set>_<test_shape>.csv
- results/figures/mlp_synthetic_plus_real_clean_confusion_matrix_<feature_set>_<test_shape>.png
- results/figures/mlp_synthetic_plus_real_clean_loss_curve_<feature_set>_<test_shape>.png
...
INPUT_CSV = BASE_DIR / "results" / "learning_data_synthetic_plus_real_curated_clean" / "all_correspondences.csv"
...
def infer_shape_name(sample_id):
    ...
    # Real CAD/scan registration samples
...
def train_and_evaluate_one_split(df, feature_set_name, features, test_shape, device):
    ...
    history_path = TABLE_DIR / f"mlp_synthetic_plus_real_clean_training_history_{feature_set_name}_{test_shape}.csv"
    history_df.to_csv(history_path, index=False)

    loss_curve_path = FIGURE_DIR / f"mlp_synthetic_plus_real_clean_loss_curve_{feature_set_name}_{test_shape}.png"
    plot_loss_curve(
        history_df,
    ...
    cm_path = FIGURE_DIR / f"mlp_synthetic_plus_real_clean_confusion_matrix_{feature_set_name}_{test_shape}.png"
    ...
    result_row = {
        "model": "MLP_synthetic_plus_real_clean",
...
def main():
    ...
    results_path = TABLE_DIR / "mlp_synthetic_plus_real_clean_results_by_shape.csv"
    results_df.to_csv(results_path, index=False)

    average_df = summarize_results(results_df)

    average_path = TABLE_DIR / "mlp_synthetic_plus_real_clean_results_average.csv"
