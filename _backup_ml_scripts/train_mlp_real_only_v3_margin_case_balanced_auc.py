...
- hard 0/1 labels
- ambiguous-label removal
- case-balanced + class-balanced training
- fixed pos_weight = 1.5
- LR = 3e-4
- dropout = 0
...
Test evaluation:
    evaluated on the original target_weight labels from the dataset,
    so results remain comparable with previous experiments.
"""
...
TEST_NAME = "mlp_real_only_v3_margin_case_balanced_auc"
OUT_DIR = BASE_DIR / "results" / "by_test" / TEST_NAME

MODEL_NAME = "MLP_real_only_v3_margin_case_balanced_auc"
...
def sample_case_class_balanced(train_df, rows_per_case, seed):
    """
    For each real case:
    - sample roughly 50% positive and 50% negative according to MARGIN_TARGET
    - if one class is missing, sample from the available class
    ...
    for case_idx, case_id in enumerate(case_ids):
        ...
        pos_df = case_df[case_df[MARGIN_TARGET] == 1]
        neg_df = case_df[case_df[MARGIN_TARGET] == 0]

        n_pos_target = rows_per_case // 2
        n_neg_target = rows_per_case - n_pos_target

        parts = []

        if len(pos_df) > 0 and len(neg_df) > 0:
            pos_replace = len(pos_df) < n_pos_target
            neg_replace = len(neg_df) < n_neg_target

            parts.append(
                pos_df.sample(
                    n=n_pos_target,
                    replace=pos_replace,
                    random_state=seed + 1009 * case_idx + 1,
                )
            )
            ...
            parts.append(
                neg_df.sample(
                    n=n_neg_target,
                    replace=neg_replace,
                    random_state=seed + 1009 * case_idx + 2,
                )
            )

            sampled_with_replacement = pos_replace or neg_replace

        elif len(pos_df) > 0:
            replace = len(pos_df) < rows_per_case
            parts.append(
                pos_df.sample(
                    n=rows_per_case,
                    replace=replace,
                    random_state=seed + 1009 * case_idx + 3,
                )
            )
            sampled_with_replacement = replace

        elif len(neg_df) > 0:
            replace = len(neg_df) < rows_per_case
            parts.append(
                neg_df.sample(
                    n=rows_per_case,
                    replace=replace,
                    random_state=seed + 1009 * case_idx + 4,
                )
            )
            sampled_with_replacement = replace
        ...
        sampled_case = pd.concat(parts, axis=0, ignore_index=True)
        ...
        count_rows.append({
            ...
            "original_positive": len(pos_df),
            "original_negative": len(neg_df),
            "original_positive_rate": float(case_df[MARGIN_TARGET].mean()) if len(case_df) else np.nan,
...
def train_one_fold(train_margin_df, val_margin_df, test_df, heldout_case, device, fold_seed):
    scaler = StandardScaler()
    scaler.fit(train_margin_df[FEATURES].values)

    X_test = scaler.transform(test_df[FEATURES].values)
    y_test_original = test_df[HARD_TARGET].values.astype(int)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        sampled_train_df, sample_counts_df = sample_case_class_balanced(
    ...
    result_row = {
        ...
        "test_original_positive_rate": float(test_df[HARD_TARGET].mean()),
    }
    ...
    return result_row, threshold_rows, history_rows, sampling_rows
...
def main():
    ...
    print(f"case-balanced rows per case per epoch = {ROWS_PER_CASE_PER_EPOCH}")
    ...
    for heldout_index, heldout_case in enumerate(real_case_ids):
        ...
        split_rows.append({
            ...
            "train_margin_rows": len(train_margin_df),
            "val_margin_rows": len(val_margin_df),
            "test_original_rows": len(test_df),
            "train_margin_positive_rate": float(train_margin_df[MARGIN_TARGET].mean()),
            "val_margin_positive_rate": float(val_margin_df[MARGIN_TARGET].mean()),
            "test_original_positive_rate": float(test_df[HARD_TARGET].mean()),
        })
        ...
        result_row, threshold_rows, history_rows, sampling_rows = train_one_fold(
        ...
        print(
            ...
            f"acceptance={result_row['acceptance_rate']:.3f} | "
            f"best_epoch={result_row['best_epoch']} | "
            f"best_val_auc={result_row['best_val_auc_margin']:.3f}"
        )
